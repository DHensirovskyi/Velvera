...
export const runtime = 'nodejs'; // важненько: не edge

const resend = new Resend(process.env.RESEND_API_KEY!);
const FROM = process.env.MAIL_FROM || 'Velvera <onboarding@resend.dev>';

/** Отдаёт базовый URL:
 *  1) SITE_URL из env (без завершающего /)
 *  2) иначе собираем из заголовков запроса (работает на превью/локали)
 */
...
export async function POST(req: Request) {
  try {
    ...
    const cancelUrl = `${baseUrl}/cancelled.html?slot=${encodeURIComponent(slot)}`;

    // форматируем дату по-немецки (DD.MM.YYYY)
    ...
    const preheader =
      `Termin ${dateHuman} ${time} bestätigt. Falls nötig, hier stornieren.`;
...
<body style="margin:0;background:#f6f7fb">
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;background:#ffffff;border-radius:16px;box-shadow:0 6px 24px rgba(18,18,23,.06);overflow:hidden;font-family:Inter,system-ui,Segoe UI,Roboto,Arial,sans-serif;color:#111827">
        <tr><td style="padding:24px">
          ...
          <p style="margin:0 0 12px">
            Hallo, <b>${escapeHtml(name)}</b>! Ihr Termin <b>${escapeHtml(service)}</b>
            ist reserviert am <b>${dateHuman} ${time}</b>.
          </p>
          ...
          <p style="margin:0;color:#6b7280;font-size:12px">
            Wenn Sie doch kommen können — alles gut, der Termin bleibt für Sie reserviert.
          </p>

          <!-- Текстовый линк на случай, если кнопку всё равно не видно -->
    ...
    // plain-text альтернатива (улучшает доставляемость)
    const text =
      `Terminbestätigung\n\n` +
      `Hallo, ${name}!\n` +
      `Ihr Termin ${service} ist reserviert am ${dateHuman} ${time}.\n\n` +
